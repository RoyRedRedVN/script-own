...
local namTbl = {
    ...
    "67", "Agarrini La Palini", "Trulimero Trulicina", "Wardenelli Brickatoni"
}
...
local evtSec = autoTab:Section({ Title = "Auto Event", Icon end
})
evtSec:Toggle({
...
local toolsInfo = {
    ...
    ["Banana Gun"] = {time ="] = {time = 0.33, mode = "single"},
}
local toolNames = {}
...
sellSec:Toggle({
    ...
    Value = false,
    Callback = function(state) getgenv().AllowSellInventoryFull = state Icon = "trash-2",
...
eggSec:Dropdown({
    Title = "Choose Egg",
    Values = eggTblv().AutoOpenEgg = state end
})
...
local function parseMoney(s)
    ...
    for suf,m in pairs(mult) do
        if s:find(suf) then
            local num = tonumber(s:match("([%d%.]+)"))
            return num and num*m or 0
        end
    end
...
local function brName(br)
    ...
    if title then
        ...
        if title:IsA("StringValue") then
            text = title.Value
        end
    end
    if not text or text == "" then return nil end
    local names = namTbl or {}
...
local function findBestTarget()
    ...
    for _, m in ipairs(brFolder:GetChildren()) do
        ...
        if isBoss then
            push(candidates.bosses, m, rarity, name)
            continue
        end
        if next(nameFilter) and nameFilter[name] then
            push(candidates.byName, m, rarity, name)
            continue
        end
        if next(rarFilter) then
            ...
            if rf then
                for rarName in pairs(rarFilter) do
                    if rf:FindFirstChild(rarName) then
                        push(candidates.byRarity, m, rarity, name)
                        goto continueLoop
                    end
                end
            end
    ...
    if getgenv().AutoAttackBrainrotOnlyBoss then
        local boss = candidates.bosses[1]
        return boss and boss.model, boss and boss.name
    end
    local priorityOrder = {
        candidates.bosses,
        candidates.byName,
    ...
    for _, group in ipairs(priorityOrder) do
        if #group > 0 then
            return group[1].model, group[1].name
        end
    end
    return nil, nil
end
    ...
    while true do
        ...
        if getgenv().AutoBuyBestGear and gearTbl and #gearTbl>0 and buyGearRem then
            ...
            for i = math.max(1,n-1), n do
                sFire(buyGearRem, gearTbl[i], true)
                task.wait(cfg.BUY_DELAY)
            end
        end
    end
    ...
    while true do
        ...
        pcall(function()
            ...
            if moneyLabel and complete.Visible then
                local required = parseMoney(moneyLabel.Text)
        ...
        pcall(function()
            ...
            if not target then
                for _,it in ipairs(bp:GetChildren()) do
                    if it:IsA("Tool") and it:GetAttribute("ItemName") == wantedName then
                        target = it
                        break
                    end
                end
            ...
            if target then
                if target.Parent == bp then
                    hum:EquipTool(target)
                    task.wait(0.05)
                end
                local interact = remotesRoot and remotesRoot:FindFirstChild("Events") and remotesRoot.Events:FindFirstChild("Prison") and remotesRoot.Events.Prison:FindFirstChild("Interact")
                if interact then
                    sFire(interact, "TurnIn")
                end
                task.wait(0.1)
            end
        end)
...
local function getUseItemRemote()
    return remotesRoot and remotesRoot:FindFirstChild("UseItem")
end
...
task.spawn(function()
    while task.wait(0.1) do
        ...
        for _, toolName in ipairs(toolsSel) do
            ...
            if tool then
                usableTool, usableInfo, usableName = tool, toolsInfo[toolName], toolName
                break
            end
        end
...
spawn(function()
    ...
    while true do
        ...
        if shouldSell and sellRem then
            pcall(function()
                sFire(sellRem, nil)
            end)
        end
